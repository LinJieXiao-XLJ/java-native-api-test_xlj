...
public class TestInsert_V1_Error extends BaseTestSuite_TableModel {
    ...
    public Iterator<Object[]> getData() throws IOException {
        return new CustomDataProvider().load_table("data/tableModel/insert_error.csv", true).getData();
    }
...
//    @Test(priority = 10) // 测试执行的优先级为10
    public void insert() throws IoTDBConnectionException, StatementExecutionException, IOException {
        // 获取解析后的数据
        for (Iterator<Object[]> it = getData(); it.hasNext(); ) {
            ...
            for (Object insertSQL : insertSQLs) {
                try {
                    ...
                    // 若不符合预期则断言
                    assert "org.apache.iotdb.rpc.StatementExecutionException".equals(e.getClass().getName()) : "TestInsert_V1_Error 的 insert 测试失败-其他错误:" + e;
                }
            }
        }
    }
    ...
    public void insertRelationalTablet() throws IoTDBConnectionException, StatementExecutionException, IOException {
        ...
        try {
            ...
            System.out.println(e);
            // 判断是否符合预期
//            assert "java.lang.NumberFormatException: For input string: \"0.0\"".equals(e) : "错误不符合预期：" + e;
        }
    }
